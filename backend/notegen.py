...
def create_system_prompt_for_text_input(options):
    """
    Creates and returns a system prompt string based on the contents of the given 'options'.

    Args:
        options: A dictionary representing user-selected options for note generation.  Keys and possible values below:
    ...
    """
    system_prompt = 'Your task is to generate notes from the transcribed text, adhering to the following guidelines: The notes should focus solely on the content itself, omitting any references to the lecture or the speaker. '

    # format ('bullet' / 'plain')
    if options['format'] == 'bullet':
        system_prompt += 'Generate notes in bullet-point format. '
    else:
        system_prompt += 'Generate notes in paragraph-form with full sentences, rather than bullet-points. '

    # headings (True / False) -- if "format" is 'paragraph', "headings" should be FALSE
    if options['headings'] == True:
        system_prompt += 'Organize the notes with headings and/or subheadings. '
    else:
        system_prompt += 'Organize the notes without headings and/or subheadings. '

    # detail-level? ('detailed' / 'ßmedium' / 'brief')
    if options['detail'] == 'high':
        system_prompt += 'The detail level should be high; Include all relevant points and examples. '
    elif options['detail'] == 'medium':
        system_prompt += 'The detail level should be medium; Summarize key points, omitting some details. '
    elif options['detail'] == 'low':
        system_prompt += 'The detail level should be low; Capture only the main ideas and key concepts. '
...
#     # 'Complex' transcription - no enhancements
#     # generate_transcription('inputs/en-complex.wav')

#     # 'Complex' transcription - enhanced using prompt parameter
#     # generate_transcription('inputs/en-complex.wav',
...
#     # 'Complex' transcription - enhanced using post-processing
